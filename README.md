...
```
$ curl -i -H "Accept: text/plain" http://localhost:8080/txt2png?input=foo%40example.com&background=white
```
Use the returned Base64 string in HTML like this:
```html
...
### Binary output
```
$ curl -i http://localhost:8080/txt2png?input=foo%40example.com&background=white
```
(If you want, you can optionally specify a header of `Accept: image/png`.)

HTML:
```html
<img src="http://localhost:8080/txt2png?input=foo%40example.com&background=white" />
```

Markdown:
```markdown
![alt text](http://localhost:8080/txt2png?input=foo%40example.com&background=white)
```
...
Also note that any attempts to obtain security or privacy through obscurity are ineffective against any sufficiently determined attacker. This tool merely raises the threshold of effort required to harvest mildly sensitive data (such as email addresses). Please do not confuse this tool for any form of cryptography.

## Hosting
A hosted version is available here: LINK TODO (maximum input length per image: 280 characters). The hosted service is provided by a hobbyist, and is free for reasonable private use.
...
For commerical use, please host this yourself and consider making a donation. (donate button)
...
When running locally, you can access the test harness in your browser at `http://localhost:8080`.
